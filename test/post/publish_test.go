...
func TestNormalPublish(t *testing.T) {
	...
	test.CreateAccount(t, newAccountName, lb, 0,
		secp256k1.GenPrivKey(), newAccountTransactionPriv, "100")

	test.CreateTestPost(
		t, lb, newAccountName, postID1, 1, newAccountTransactionPriv, baseTime)
	test.SimulateOneBlock(lb, baseTime+test.PostIntervalSec)
	test.CreateTestPost(
		t, lb, newAccountName, postID2, 2, newAccountTransactionPriv, baseTime+test.PostIntervalSec)
}
