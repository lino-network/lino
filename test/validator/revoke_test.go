...
func TestRevoke(t *testing.T) {
	...
	test.CreateAccountWithTime(t, newAccountName, lb, uint64(0),
		newAccountResetPriv, newAccountTransactionPriv, "500000", baseTime)
	...
	test.SignCheckDeliver(t, lb, valRegisterMsg, 2, true, newAccountTransactionPriv, baseTime)
	...
	test.CreateAccountWithTime(t, newVoterName, lb, uint64(1),
		newVoterResetPriv, newVoterTransactionPriv, "500000", baseTime)
	voterDepositMsg := types.NewStakeInMsg(newVoterName, linotypes.LNO("100000"))
	test.SignCheckDeliver(t, lb, voterDepositMsg, 1, true, newVoterTransactionPriv, baseTime)

	// let voter vote validator1
	voteMsg := valtypes.NewVoteValidatorMsg(newVoterName, []string{"validator1"})
	test.SignCheckDeliver(t, lb, voteMsg, 2, true, newVoterTransactionPriv, baseTime)
	...
	test.SignCheckDeliver(t, lb, valRegisterMsg, 4, false, newAccountTransactionPriv, baseTime)
	test.CheckOncallValidatorList(t, newAccountName, false, lb)

	test.SignCheckDeliver(t, lb, valRevokeMsg, 5, false, newAccountTransactionPriv, baseTime)
	test.CheckOncallValidatorList(t, newAccountName, false, lb)
	...
	test.SignCheckDeliver(t, lb, valRegisterMsg, 6, true, newAccountTransactionPriv, baseTime+param.ValidatorRevokePendingSec+2)
	test.CheckOncallValidatorList(t, newAccountName, true, lb)
