...
func TestValidatorRevoke(t *testing.T) {
	...
	test.CreateAccount(t, newAccountName, lb, 0,
		newAccountMasterPriv, newAccountTransactionPriv, newAccountPostPriv, "500000")
	...
	test.CheckBalance(t, newAccountName, lb, types.NewCoin(50000*types.Decimals))
	// check the first coin return
	test.SimulateOneBlock(lb, baseTime+test.CoinReturnIntervalHr*3600+1)
	test.CheckBalance(t, newAccountName, lb, types.NewCoin(7142857143))
	...
	test.CheckBalance(t, newAccountName, lb, types.NewCoin(200000*types.Decimals))
