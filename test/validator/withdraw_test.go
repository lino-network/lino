...
func TestValidatorRevoke(t *testing.T) {
	...
	voteDepositMsg := types.NewStakeInMsg(newAccountName, types.LNO("300000"))
	...
	test.CheckBalance(t, newAccountName, lb, linotypes.NewCoinFromInt64(49999*linotypes.Decimals))
	// check the first coin return
	test.SimulateOneBlock(lb, baseTime+test.CoinReturnIntervalSec+1)
	test.CheckBalance(t, newAccountName, lb, linotypes.NewCoinFromInt64(7142757143))
	...
	test.CheckBalance(t, newAccountName, lb, linotypes.NewCoinFromInt64(199999*linotypes.Decimals))
