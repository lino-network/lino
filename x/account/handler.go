...
func handleTransferMsg(ctx sdk.Context, am AccountManager, msg TransferMsg) sdk.Result {
	...
	if err := am.MinusSavingCoin(
		ctx, msg.Sender, coin, string(msg.Receiver), msg.Memo, types.TransferOut); err != nil {
		return err.Result()
	}

	// send coins using username
	if err := am.AddSavingCoin(
		ctx, msg.Receiver, coin, string(msg.Sender), msg.Memo, types.TransferIn); err != nil {
...
func handleRegisterMsg(ctx sdk.Context, am AccountManager, msg RegisterMsg) sdk.Result {
	...
	if err := am.MinusSavingCoin(
		ctx, msg.Referrer, coin, string(msg.NewUser), "", types.TransferOut); err != nil {
		return err.Result()
	}
