...
type DecideProposalEvent struct {
	ProposalType linotypes.ProposalType `json:"proposal_type"`
	ProposalID   linotypes.ProposalKey  `json:"proposal_id"`
}
...
func (dpe DecideProposalEvent) Execute(
	...
	if !proposalManager.IsOngoingProposal(ctx, dpe.ProposalID) {
		return types.ErrOngoingProposalNotFound()
	}
	...
	if proposalRes == linotypes.ProposalNotPass {
		return nil
	}
...
func (dpe DecideProposalEvent) ExecuteChangeParam(
	ctx sdk.Context, curID linotypes.ProposalKey, proposalManager ProposalManager,
	gm *global.GlobalManager) sdk.Error {
	...
	if err := gm.RegisterEventAtTime(ctx,
		ctx.BlockHeader().Time.Unix()+linotypes.ParamChangeTimeout, event); err != nil {
		return err
	}
	return nil
}
...
func (dpe DecideProposalEvent) ExecuteContentCensorship(
	...
	if exist := postManager.DoesPostExist(ctx, permlink); !exist {
		return types.ErrCensorshipPostNotFound()
	}
...
func (dpe DecideProposalEvent) ExecuteProtocolUpgrade(
	ctx sdk.Context, curID linotypes.ProposalKey, proposalManager ProposalManager) sdk.Error {
	return nil
}
