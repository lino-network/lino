...
func TestRemoveDelegatee(t *testing.T) {
	ctx, am, vm, _ := setupTest(t, 0)
	minBalance := types.NewCoinFromInt64(1 * types.Decimals)
	user1 := createTestAccount(ctx, am, "user1", minBalance)
	cases := []struct {
		testName         string
		beforeRemove     model.DelegateeList
		usernameToRemove types.AccountKey
		expectResult     sdk.Error
		expectList       model.DelegateeList
	}{
		{"remove only delegatee",
			model.DelegateeList{[]types.AccountKey{"test"}}, types.AccountKey("test"), nil,
			model.DelegateeList{[]types.AccountKey(nil)},
		},
		{"remove non exist delegatee",
			model.DelegateeList{[]types.AccountKey{"test1"}}, types.AccountKey("test"), nil,
			model.DelegateeList{[]types.AccountKey{"test1"}},
		},
		{"remove one of delegatee",
			model.DelegateeList{[]types.AccountKey{"test1", "test2", "test3"}},
			types.AccountKey("test3"), nil,
			model.DelegateeList{[]types.AccountKey{"test1", "test2"}},
		},
	}

	for _, cs := range cases {
		err := vm.storage.SetDelegateeList(ctx, user1, &cs.beforeRemove)
		assert.Nil(t, err)
		err = vm.RemoveDelegatee(ctx, user1, cs.usernameToRemove)
		assert.Nil(t, err)
		delegateeLst, err := vm.storage.GetDelegateeList(ctx, user1)
		assert.Nil(t, err)
		assert.Equal(t, cs.expectList, *delegateeLst)
	}
}

func TestAddDelegatee(t *testing.T) {
	ctx, am, vm, _ := setupTest(t, 0)
	minBalance := types.NewCoinFromInt64(1 * types.Decimals)
	user1 := createTestAccount(ctx, am, "user1", minBalance)
	cases := []struct {
		testName      string
		beforeAdding  model.DelegateeList
		usernameToAdd types.AccountKey
		expectResult  sdk.Error
		expectList    model.DelegateeList
	}{
		{"add delegatee to empty lst",
			model.DelegateeList{}, types.AccountKey("test"), nil,
			model.DelegateeList{[]types.AccountKey{"test"}},
		},
		{"add delegatee to the end of lst",
			model.DelegateeList{[]types.AccountKey{"test1", "test2"}},
			types.AccountKey("test"), nil,
			model.DelegateeList{[]types.AccountKey{"test1", "test2", "test"}},
		},
	}

	for _, cs := range cases {
		err := vm.storage.SetDelegateeList(ctx, user1, &cs.beforeAdding)
		assert.Nil(t, err)
		err = vm.AddDelegatee(ctx, user1, cs.usernameToAdd)
		assert.Nil(t, err)
		delegateeLst, err := vm.storage.GetDelegateeList(ctx, user1)
		assert.Nil(t, err)
		assert.Equal(t, cs.expectList, *delegateeLst)
